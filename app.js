...
function executeVendiSimulatedCheckout() {
    ...
    const steps = [
        "カード情報を安全に検証しています...",
        "独占オーナーシップ契約書を生成中...",
        "VENDIX 命名権アクティベーションコードを生成しています...",
...
async function fetchWithTimeout(url, options = {}, timeout = 3000) {
    ...
    try {
        const response = await fetch(url, {
            ...
            headers: {
                'User-Agent': 'VendixApp/1.0 (contact@aoisoftware.com)',
                'Accept-Language': 'ja',
                ...(options.headers || {})
            }
        });
            ...
            if (res.ok) {
                ...
                if (data.status === 'success' && data.paid) {
                    ...
                    if (target) {
                        ...
                        const autoKey = `LS-VENDIX-${sessionId.replace('mock_session_', '').substring(0, 10).toUpperCase()}`;
