        ...
        contents: [
            {
                ...
                parts: [
                    {
                        text: "あなたは自販機の画像から商品のラインナップを詳細に判別する世界トップクラスの『自販機分析AI』です。この画像に写っている飲料商品を識別し、各飲料の名前、ホットかコールドかの属性、販売価格、そこで「売切」であるかどうかを解析してください。売り切れの商品（売切ランプ点灯、売切ラベル表示、または商品見本がないスロット）には必ず `[売切]` を含めてください。必ず以下のプレーンなJSON形式の配列のみで返してください。マークダウンによる説明、注釈、あるいは会話的応答は一切排除してください。\n\n`[\"天然水 [コールド] (130円)\", \"BOSS 缶コーヒー [ホット] [売切] (140円)\", \"コカ・コーラ [コールド] (160円)\"]` を参考に、エスケープ（\\）を付けずに生のダブルクォーテーションを使用して `[\"天然水 [コールド] (130円)\"]` ではなく `[\"天然水 [コールド] (130円)\"]`（文字列内のダブルクォートのみ）を正しくパースできる配列の形式で出力してください。"
                    },
                    {
                        inlineData: {
                            mimeType: "image/jpeg",
